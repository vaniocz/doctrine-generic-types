...
$cacheDirectory = defined('VANIO_DOCTRINE_GENERIC_TYPES_CACHE_DIRECTORY')
    ...
    : sys_get_temp_dir();
$originalFile = ComposerUtility::findClassFileUsingPsr(Type::class);
$patchedFile = sprintf(
    '%s/%s_%s_%s.php',
    $cacheDirectory,
...
if (!is_readable($patchedFile)) {
    $code = preg_replace(
        '~function\s+getTypeRegistry\s*\(.*\)\s*:\s*TypeRegistry\s*{~',
        'function getTypeRegistry(): TypeRegistry|\Vanio\DoctrineGenericTypes\DBAL\GenericTypeRegistry {',
